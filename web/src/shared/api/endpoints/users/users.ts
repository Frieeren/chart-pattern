...
export const readUsersUsersGet = (params?: ReadUsersUsersGetParams, signal?: AbortSignal) => {
  return httpClient<UserResponse[]>({ url: 'http://localhost:3000/api/users/', method: 'GET', params, signal });
};

export const getReadUsersUsersGetQueryKey = (params?: ReadUsersUsersGetParams) => {
  return ['http://localhost:3000/api/users/', ...(params ? [params] : [])] as const;
};
...
export const readUserUsersUserIdGet = (userId: number, signal?: AbortSignal) => {
  return httpClient<UserResponse>({ url: `http://localhost:3000/api/users/${userId}`, method: 'GET', signal });
};

export const getReadUserUsersUserIdGetQueryKey = (userId: number) => {
  return [`http://localhost:3000/api/users/${userId}`] as const;
};
